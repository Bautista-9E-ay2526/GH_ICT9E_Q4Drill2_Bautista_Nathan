function checkHeat() {
    let temperature = Number(document.getElementById("Degree").value);
    let humidity = Number(document.getElementById("humid").value);
    let ans = document.getElementById("Ans");
    let heatIndex = temperature + (0.33 * humidity) - 4;

    let status = "";
    let description = "";

    if (document.getElementById("Degree").value === "" || document.getElementById("humid").value === "") {
        window.alert("Please fill in both fields.");
        return;
    }

    if (heatIndex <= 25.9) {
        status = "Comfortable";
    } 
    else if (heatIndex >= 28 && heatIndex <= 32.9) {
        status = "Warm";
    } 
    else if (heatIndex >= 33 && heatIndex <= 37.9) {
        status = "Hot";
    } 
    else if (heatIndex >= 38 && heatIndex <= 41.9) {
        status = "Very Hot";
    } 
    else if (heatIndex >= 42) {
        status = "Extreme Heat";
    }

    window.alert("The Heat Index is: " + heatIndex.toFixed(2) + "°C\nStatus: " + status);
}